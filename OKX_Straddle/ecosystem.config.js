...
// 15 10 * * * HOME=/root /usr/local/bin/pm2 restart okx-reporting-prod >> /root/cex_option_trading/OKX_Straddle/data/logs/cron.log 2>&1
...
// Checks:
//   pm2 show okx-reporting-prod                      # check "cron restart" field
//   To smoke-test the report without waiting until 09:00 UTC, just pm2 restart okx-reporting-prod — it'll run one cycle and go back to "stopped" status, which is the expected idle state for cron jobs.
